...
import { AbstractControl, ControlConfig, FormArray, FormBuilder, FormControl, FormGroup, ReactiveFormsModule, ValidationErrors, ValidatorFn, Validators } from '@angular/forms';
import { timestamp } from 'rxjs';

type ShippingMethod = "courier" | "pickup" | null

interface Cart { //я въебал это, чтобы работало заполнение демо
  customer: {
    name: string;
    email: string;
    phone: string;
...
export class CartForm {
  ...
  private noTempDomen: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    ...
    return badMail.includes(control.value.slice(control.value.lastIndexOf('@')).toLowerCase().trim())? {noTempDomen: true} : null //не ввел переменную, потому что не придумал, как назвать. зачем тут трим я снова не понимаю, типа по приколу от пробелов на концах?
  }
  ...
  private phoneLike: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    ...
    return (control.value === "" || (isPhoneGood && phoneLength > 6)) ? null : {phoneLike: true};
  }
  ...
  private naturalNumber: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    return (control.value === null || ((control.value >= 1) && Number.isInteger(Number(control.value)))) ? null : {naturalNumber: true};
  }
  ...
  private minItems = (min: number): ValidatorFn => (array: AbstractControl): ValidationErrors | null => {
    const itemsArray = array as FormArray
    return itemsArray.length < min ? {minItems: true} : null
  }

  private maxTotal = (max: number): ValidatorFn => (array: AbstractControl): ValidationErrors | null => {
    const formArray = array as FormArray
    //сначала я это сделал, потом с ретурном разбирался
    // for (const item of formArray.controls) { 
    //   total += Number(item.value.price) * Number(item.value.qty)
    // }
    return formArray.value.reduce((sum: number, item: { price: number; qty: number; }) => sum + item.price * item.qty, 0) > max ? {maxTotal: true} : null
  }
  ...
  private zipFormat: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    ...
    return (control.value === "" || (isZipFormatGood && zipLength > 4 && zipLength < 11)) ? null : {zipFormat: true};
  }
  
  //не работает
  private dateFormat: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    const date = new Date(control.value);
    return (control.value !== "" && date.toString() === 'Invalid Date') ? {dateFormat: true} : null
  }

  //не работает
  private dateWithin = (minDays: number, maxDays: number): ValidatorFn => (control: AbstractControl): ValidationErrors | null => {
    ...
    return (!isNaN(shippingDate.getTime()) && (shippingDate < min || shippingDate > max) ) ? {dateWithin: true} : null ;
  }

  private notWeekend: ValidatorFn = (control: AbstractControl): ValidationErrors | null => {
    return (new Date(control.value).getDay() === 0 || new Date(control.value).getDay() === 6) ? {notWeekend: true} : null ;
  }
  ...
  cartForm = this.fb.group({
    ...
    customer: this.fb.group({
      ...
      email: this.fb.nonNullable.control<string>("", [Validators.required, Validators.email, this.noTempDomen]),
      phone: this.fb.nonNullable.control<string>("", [Validators.required, this.phoneLike])

    }),

    items: this.fb.array([this.itemFormGroup()], [this.minItems(1), this.uniqueSkus, this.maxTotal(10000)]),
    ...
    shipping: this.fb.group({
      ...
      date: this.fb.control<string>("", [Validators.required, this.dateFormat, this.dateWithin(3, 14), this.notWeekend]) //не понял, что за дата - решил, что и для доставки, и для саомвывоза актуальна, плюс навесил на нее обязательность, плюс добавил проверку на формат
...
//Я ничего не понимаю
  constructor() {//Я ничего не понимаю
  ...
  removeItem(index: number) {
    if (this.cartForm.controls.items.length > 1) {this.cartForm.controls.items.removeAt(index)}
  }
  ...
  updateValidators() { //я все еще ничего не понимаю
    if (this.cartForm.controls.shipping.controls.method.value === "courier") {
      this.cartForm.controls.shipping.controls.address.controls.city.setValidators([Validators.required]);
      this.cartForm.controls.shipping.controls.address.controls.street.setValidators([Validators.required]);
      this.cartForm.controls.shipping.controls.address.controls.zip.setValidators([Validators.required, this.zipFormat]);
      this.cartForm.controls.shipping.controls.pickupPoint.clearValidators();
      this.cartForm.controls.shipping.controls.address.controls.city.updateValueAndValidity({ emitEvent: false });
      this.cartForm.controls.shipping.controls.address.controls.street.updateValueAndValidity({ emitEvent: false });
      this.cartForm.controls.shipping.controls.address.controls.zip.updateValueAndValidity({ emitEvent: false });
      this.cartForm.controls.shipping.controls.pickupPoint.updateValueAndValidity({ emitEvent: false });
    }
      ...
      this.cartForm.controls.shipping.controls.pickupPoint.setValidators([Validators.required])
      this.cartForm.controls.shipping.controls.address.controls.city.updateValueAndValidity({ emitEvent: false });
      this.cartForm.controls.shipping.controls.address.controls.street.updateValueAndValidity({ emitEvent: false });
      this.cartForm.controls.shipping.controls.address.controls.zip.updateValueAndValidity({ emitEvent: false });
      this.cartForm.controls.shipping.controls.pickupPoint.updateValueAndValidity({ emitEvent: false });
  ...
  onSubmit() {
    ...
    console.log(this.cartForm.controls.items.value.reduce((sum, item) => sum + Number(item.price) * Number(item.qty), 0))
    if (this.cartForm.valid === false) {this.cartForm.markAllAsTouched()};
  }
}
